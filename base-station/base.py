...
# Setting up Redis client for pub/sub communication
try:
    redis_client = redis.Redis()
    redis_client.ping()  # To test redis connection
...
# Optional cantools import and DBC loading for CAN message decoding
try:
    import cantools
...
# Memory safeguards for batching CAN frames
...
# UDP listener socket setup for receiving CAN JSON messages
...
# Use deque with maxlen for automatic memory management of batches
...
def print_stats():
    ...
    while True:
        ...
        print(
            f"Pipe writes: {stats['messages_published_success']} success, {stats['messages_published_failed']} failed"
        )
        print(
            f"HTTP forwards: {stats['http_forwards_success']} success, {stats['http_forwards_failed']} failed"
        )
...
def open_pipe():
    ...
    try:
        if pipe_fd is not None:
            return True
        pipe_fd = os.open(
            NAMED_PIPE_PATH, os.O_WRONLY | os.O_NONBLOCK
        )  # Uncomment on Base Station / comment out when testing on macOS
...
def canserver_broadcast(frames):
    """Write CAN frames to Redis Pub/Sub or Named Pipe with error handling."""
    if not frames:
        return
    ...
    try:
        if IS_REDIS_ACTIVE:
            ...
            stats["messages_published_success"] += 1
            print(f"Successfully wrote {len(frames)} frames to named pipe")
        else:
            print(
...
def broadcast_time():
    """Broadcast timestamp for ESP32 sync via UDP broadcast."""
...
def broadcast_batch_timer():
    """Periodically flush accumulated CAN frames with memory safeguards."""
    ...
    while True:
        ...
        with batch_lock:
            ...
            should_flush = (
                len(batched_frames) >= MAX_BATCH_SIZE
                or (batched_frames and current_time - last_batch_time >= MAX_BATCH_AGE)
                or len(batched_frames) > 0
            )
...
# Start background threads for time broadcast, batch flushing, and diagnostics printing
...
# Main UDP listener loop to receive and process CAN JSON messages
try:

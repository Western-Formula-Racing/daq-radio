...
# Initialize Redis client and subscribe to 'can_messages' channel
try:
    redis_client = redis.Redis()
    redis_client.ping()  # testing if connection to server is available
...
def named_pipe_listener():
    """Read CAN messages from Redis Pub/Sub or Named Pipe."""
    pipe_path = "/tmp/can_data_pipe"
    message_count = 0
    if IS_REDIS_ACTIVE:
        # Listen for messages from Redis Pub/Sub channel
        while True:
            try:
                for sub_msg in pubsub.listen():
                    ...
                    if sub_msg:
                        ...
                        if message_count % 100 == 0:
                            print(
                                f"Processed {message_count} messages from Redis Pub/Sub"
                            )
                        try:
        ...
        # Fallback to reading from named pipe if Redis is inactive
        try:
            while True:
...
def update_table(n, time_range, can_id, message_name, filter_mode):
    # Dash callback to update table data periodically based on filters
    with lock:

...
- Writes all metrics to TimescaleDB in the **`monitoring`** database (table name is hardcoded, not configurable):
...
| `HEALTH_MONITOR_TSDB_VOLUME_SUFFIX`     | `timescaledb-data`                | Volume suffix used to find the TimescaleDB data volume for disk usage. |

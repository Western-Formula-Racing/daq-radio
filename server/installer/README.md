...
On first boot, upload sample CAN data via the file-uploader web UI at `http://localhost:8084` after the stack is running.
...
| `GRAFANA_ADMIN_PASSWORD` | Grafana administrator password | `password` (see `.env.example`) |
...
| `ANTHROPIC_API_KEY` | API key for AI-powered code generation (routed through MiniMax via an Anthropic-compatible endpoint) | empty |
| `ANTHROPIC_BASE_URL` | Base URL for the Anthropic-compatible API | `https://api.minimaxi.com/anthropic` |
| `ANTHROPIC_MODEL` | Model to use for code generation | `MiniMax-M3` |
...
| `code-generator` | `3030` (internal) | AI-powered code generation service using an Anthropic-compatible API (MiniMax-M3 via MiniMax). Generates Python code from natural language. |
...
- `example.dbc` is a minimal CAN database that defines two demo messages. Replace this file with your team’s CAN definition when working with real data.
- Upload sample or production CSV files through the file-uploader at `http://localhost:8084`.
...
**Features:**
- Automatic code generation from natural language using an Anthropic-compatible API (MiniMax-M3)
...
2. Optional: Configure `ANTHROPIC_MODEL`, `ANTHROPIC_BASE_URL`, and `MAX_RETRIES`

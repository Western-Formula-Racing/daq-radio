...
Generate and execute Python code for telemetry analysis using MiniMax AI and a custom sandboxed execution environment. Integrated with the Slackbot for natural language queries.
...
flowchart TD
    ...
    subgraph "Code Generator (port 3030)"
        ...
        MiniMaxAI["MiniMax AI (MiniMax-M3)"]
        RetryNode["Append error to prompt and retry"]
        Check{"Execution successful?"}

        Receive --> LoadGuide
        LoadGuide -->|"system prompt + user prompt"| MiniMaxAI
        MiniMaxAI -->|"Generated Python code"| Check
        RetryNode -->|"Updated prompt"| MiniMaxAI
    end
...
1. **Code Generator** (`code_generator.py`)
   ...
   - Uses MiniMax AI to generate Python code
...
```bash
# Required: MiniMax API key (used via an Anthropic-compatible SDK)
ANTHROPIC_API_KEY=your-minimax-api-key-here

# Optional: API base URL (default: https://api.minimaxi.com/anthropic)
ANTHROPIC_BASE_URL=https://api.minimaxi.com/anthropic

# Optional: model (default: MiniMax-M3)

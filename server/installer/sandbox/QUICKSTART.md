...
- An Anthropic-compatible API key (MiniMax by default, get one at https://www.minimaxi.com)
...
2. **Code Generator**:
   - Receives prompt
   - Loads system prompt with TimescaleDB connection details, plus RAG context from ChromaDB
...
6. **Follow-ups**: from inside the resulting Slack thread, `!reply <instructions>` continues the conversation (`/api/generate-code-followup`, 24h/15-turn session limit), `!approve`/`:+1:` saves a golden sample (`/api/feedback`), and `!aistats` shows observability data (`/api/metrics`)
...
**"ANTHROPIC_API_KEY not found" / KeyError on startup**
...
- Verify the season table name: `DEFAULT_SEASON_TABLE=wfr26` in `.env`
- Check TimescaleDB has data, e.g. via Grafana at http://localhost:8087
- Verify the password is correct: `POSTGRES_PASSWORD` in `.env`
...
┌───────────────────────┐
│  Code Generator       │
│  (MiniMax-M3 via      │ ← System Prompt + User Prompt
│   Anthropic API)      │
└──────────┬────────────┘
...
- Maximum runtime: 120 seconds by default (configurable via SANDBOX_TIMEOUT)
- Maximum file size: 20 MB per file by default (configurable via SANDBOX_MAX_FILE_MB)
- Maximum files: 20 files by default (configurable via SANDBOX_MAX_FILES)
...
- MiniMax API Documentation: https://www.minimaxi.com/document
- Custom Sandbox Source: `installer/sandbox/`
